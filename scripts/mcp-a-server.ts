 ...
 * A 股 MCP 服务器
 *
 * 实现 MCP adapter 协议，通过新浪财经滚动新闻 API 抓取 A 股资讯。
 * 无需 API key，随时可用。
 *
 * 用法: npx ts-node scripts/mcp-a-server.ts
 *       默认端口 3102，可通过 MCP_A_PORT 环境变量覆盖
 *
 * 接口（MCP 协议）：
 *   POST /tools/fetch_news
 *   Body: { arguments: { market, limit? } }
 *   Response: { items: NewsItem[] }
 *
 *   GET /tools   → { tools: ["fetch_news"] }
...
const PORT = parseInt(process.env.MCP_A_PORT ?? '3102', 10);
...
async function fetchANews(limit = 30): Promise<Partial<NewsItem>[]> {
  try {
    ...
    console.log(`[MCP-A] 新浪财经: ${items.length} 条`);
    return items.slice(0, limit);
  } catch (err: any) {
    console.warn(`[MCP-A] 新浪财经失败: ${err.message}，尝试东方财富公告...`);
  }
  try {
    const items = await fetchEFNews();
    console.log(`[MCP-A] 东方财富公告: ${items.length} 条`);
    return items.slice(0, limit);
  } catch (err: any) {
    console.error(`[MCP-A] 备用源也失败: ${err.message}`);
    return [];
  }
}
...
app.get('/tools', (_req, res) => {
  res.json({ tools: ['fetch_news'] });
});

/** MCP fetch_news 工具 */
app.post('/tools/fetch_news', async (req, res) => {
  const args = req.body?.arguments ?? req.body ?? {};
  const market: string = args.market ?? '';
  const limit: number = args.limit ?? 30;

  console.log(`[MCP-A] fetch_news market=${market} limit=${limit}`);

  if (market !== 'a') {
    res.json({ items: [], metadata: { note: `market ${market} not supported` } });
    return;
  }

  try {
    const items = await fetchANews(limit);
    console.log(`[MCP-A] Returning ${items.length} A-stock items`);
    res.json({ items, metadata: { source: 'sina_finance', fetchedAt: new Date() } });
  } catch (err: any) {
    console.error('[MCP-A] Error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

app.get('/health', (_req, res) =>
  res.json({ status: 'ok', service: 'a-stock-mcp' }),
);

app.listen(PORT, () => {
  console.log(`\n✅ A 股 MCP 服务器运行中 → http://localhost:${PORT}`);
  console.log(`   协议: POST /tools/fetch_news  body: { arguments: { market:"a" } }`);
