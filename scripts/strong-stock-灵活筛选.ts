...
async function main() {
  console.log('[StrongStock] 执行强势股筛选...\n');
  ...
  const allSymbols = new Set([
    ...
    ...volumeData.data.map((s: any) => s.symbol),
    ...turnoverData.data.map((s: any) => s.symbol)
  ]);
  ...
  for (const symbol of allSymbols) {
    ...
    const amountWan = v?.amount || 0; // 万元
    if (amountWan < 30000) continue; // 成交额必须>=3亿
    
    const change = g?.changePercent || v?.changePercent || t?.changePercent || 0;
    const turnover = t?.turnover || 0;
    
    // 灵活条件: 涨幅>=0 或者 换手率>=0.3% 或者 成交额>=10亿
    if (change >= 0 || turnover >= 0.3 || amountWan >= 100000) {
      const reasons: string[] = [];
      if (g && g.changePercent >= 0.1) reasons.push('涨幅榜');
      if (v && v.amount >= 30000) reasons.push('成交额榜');
      if (t && t.turnover >= 0.3) reasons.push('换手率榜');
      ...
      matched.push({
        ...
        volume: amountWan / 10000,
        turnover,
        reason: reasons.join('/') || '成交额达标'
      });
    }
  }
  
  matched.sort((a, b) => b.changePercent - a.changePercent);
  const items = matched.slice(0, 15);
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    .note { background: #e3f2fd; padding: 12px 16px; border-left: 3px solid #2196f3; margin: 16px; border-radius: 4px; font-size: 13px; color: #1565c0; }
    .alert { background: #fff3e0; padding: 12px 16px; border-left: 3px solid #ff9800; margin: 16px; border-radius: 4px; font-size: 13px; color: #e65100; }
  </style></head>
  <body>
    <div class="container">
      <div class="header">
      ...
      <div class="alert">
        <strong>📊 市场观察：</strong>今日市场整体表现平淡，涨幅最高的股票仅0.2%左右。换手率活跃的股票涨幅普遍较低。
      </div>
      <div class="note"><strong>筛选条件：</strong>成交额 ≥ 3亿 (含任一榜单或换手率≥0.3%)</div>
  ...
  const info = await transporter.sendMail({
    ...
    to: '845567595@qq.com',
    subject: `[${dateStr}] 每日强势股筛选报告 - 成交额达标${items.length}只`,
    html
  });
